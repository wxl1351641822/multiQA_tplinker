...
def process(data_dir, output_dir, tokenizer, is_test, is_mq=False):
    def get_entity(entity, type, role):
        ...
        if (start == -1):
            ...
            return [type, start, end, role, entity]
    """
    Args:
    ...
    with open(data_dir, "r", encoding="utf-8") as f:
        ...
        for line in lines:
            ...
            if("spo_list" not in line):
                continue
            spo_list = line["spo_list"]

            entity = {que: [] for type, que in duie_question_templates["qa_turn1"].items()}
            relation = {que: [] for type, que in duie_question_templates["qa_turn2"].items()}
            # print(ntxt2)

            for spo in spo_list:
                rel = [spo["predicate"]]
                subject = spo["subject"]
                roledict["subject"].add(spo["subject_type"])
                if(len(subject)==0):
                    # print(spo)
                    continue
                e = get_entity(entity=subject, type=spo["subject_type"], role="subject")
                if(e != -1):
                    rel.append(e)
                    entity[duie_question_templates["qa_turn1"][spo["subject_type"]]].append(e)
                    # print(textlist[e[1]: e[2]],subject, e[-1])
                else:
                    continue
                object_type = spo["object_type"]
                object = spo["object"]
                for role, type in object_type.items():
                    roleset.add(role)
                    roledict[role].add(type)
                    e = get_entity(entity=object[role], type=type, role=role)
                    if(e != -1):
                        rel.append(e)
                        entity[duie_question_templates["qa_turn1"][type]].append(e)
                    else:
                        # if(role == "@value"):
                        #     print(spo)
                        continue
                relation[duie_question_templates["qa_turn2"][rel[0]]].append(rel)
                # print(relation)
                # subject

            new_relation ={}
            # print(duie_question_templates)

            for entity_q, e in entity.items():
                if(len(e) != 0):
                    # print(entity_q,)
                    # print(duie_q2type["qa_turn1"][entity_q])
                    if(duie_q2type["qa_turn1"][entity_q] not in duie_entity2rel):
                        continue
                    for r in duie_entity2rel[duie_q2type["qa_turn1"][entity_q]]:
                        new_relation[duie_question_templates["qa_turn2"][r]] = relation[duie_question_templates["qa_turn2"][r]]
            count_rel += len(list(new_relation.keys()))
            count_entity += len(list(entity.keys()))

            res = {"context": list(text), "qa_pairs": [entity, new_relation]}
            data.append(res)
    ...
    print(count_entity, count_rel, count_rel+count_entity)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_dir", default='/data/home/wuyuming/wxl/数据集/baidu/')
    parser.add_argument(
        "--dataset_tag", choices=["ace2004", 'ace2005', "duie"], default='duie')
    parser.add_argument("--is_test", action="store_true")
    parser.add_argument("--is_mq", type=bool, default=False,
                        help="多问题吗？")
    parser.add_argument("--pretrained_model_path",
                        default='/data/home/wuyuming/wxl/pretrained_models/bert-base-chinese')
    parser.add_argument("--output_base_dir",
                        default="./")
    args = parser.parse_args()
    # args.is_test=True
    from transformers import BertTokenizer
    for file in ["duie_train", "duie_test1", "duie_dev"]:
        data_dir = args.data_dir + file +".json"
        output_base_dir = args.output_base_dir
        if(file != "duie_train"):
            args.is_test=True
        output_dir = "{}/{}_is_mq_{}".format(output_base_dir, os.path.split(
            args.pretrained_model_path)[-1], args.is_mq)
        if(not os.path.exists(output_dir)):
            os.makedirs(output_dir)
        # print(args.pretrained_model_path)
        output_dir = output_dir + "/" + file.replace("duie_", "") + ".json"
        tokenizer = BertTokenizer.from_pretrained(args.pretrained_model_path)
        process(data_dir, output_dir, tokenizer, args.is_test, args.is_mq)
