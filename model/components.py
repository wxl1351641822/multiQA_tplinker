...
class HandshakingKernel(nn.Module):
    ...
    def forward(self, seq_hiddens,seq_mask):
        '''
        seq_hiddens: (batch_size, seq_len, hidden_size)
        return:
        ...
        for ind in range(seq_len):
            ...
            # print(hidden_each_step.shape,visible_hiddens.shape,repeat_hiddens.shape)
            mask_each_step=seq_mask[:,ind].reshape(-1,1)
            visible_mask=seq_mask[:,ind:]
            visible_mask=mask_each_step*visible_mask
            shaking_mask_list.append(visible_mask)
            # print(visible_mask.shape)
            if self.shaking_type == "cat":
                shaking_hiddens = torch.cat([repeat_hiddens, visible_hiddens], dim = -1)#[batch_size,seq_len-ind,hidden_size*2=768*2]
                shaking_hiddens = torch.tanh(self.combine_fc(shaking_hiddens))#[batch_size,seq_len-ind,hidden_size=768]
        ...
        long_shaking_mask=torch.cat(shaking_mask_list,dim=1)
        # print(long_shaking_hiddens.shape,long_shaking_mask.shape)
        # for i in range(10):
        #     print(seq_mask[i].shape,sum(seq_mask[i]),torch.sum(seq_mask[i]),torch.sum(long_shaking_mask[i]),sum(long_shaking_mask[i]))
        # long_shaking_hiddens=long_shaking_hiddens[long_shaking_mask==1]

        # print(long_shaking_hiddens.shape)
        return long_shaking_hiddens,long_shaking_mask